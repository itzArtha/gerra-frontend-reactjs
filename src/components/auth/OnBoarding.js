...
const OnBoarding = ({callback}) => {
    ...
    return (<Layout>
        <React.Fragment>
            ...
            <div className="p-2 grid grid-cols-1 md:grid-cols-2 gap-2 mb-4">
                <MainBox onClick={() => {
                    path === "/login" ? history.push("/user/login") : callback("user")
                }}>Login sebagai pengguna</MainBox>
                <MainBox onClick={() => {
                    path === "/login" ? history.push("/organization/login") : callback("organization")
