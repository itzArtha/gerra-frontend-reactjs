...
const ManageEvent = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllCategoryFormat = async () => {
      await apiClient
        .get("/api/v1/organization/event/category")
        .then((response) => {
  ...
  const handleSubmit = () => {
    switch (true) {
      ...
      case showLocationModal:
        if (formData.streamUrl || formData.location) {
          handleUpdateLocation();
        } else if (!isOnline && !formData.location) {
          setFormData({
            ...formData,
      ...
      case showTicketModal || showEditTicketModal:
        if (
          ...
          let amount = formData.priceTicket.replace(/\D/g, "");
          const dataUpload = {
